...
struct SickoCrosser4LWidget : ModuleWidget {
	SickoCrosser4LWidget(SickoCrosser4L* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/SickoCrosser4L.svg")));
		...
		for (int i = 0; i < 4; i++) {
			addParam(createParamCentered<CKSSThreeHorizontal>(mm2px(Vec(xs+(x*i), ySw)), module, SickoCrosser4L::IN_SW_SWITCH+i));
			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xs+(x*i), yIn1)), module, SickoCrosser4L::IN_A_INPUT+i));
			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xs+(x*i), yIn2)), module, SickoCrosser4L::IN_B_INPUT+i));
			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xs+(x*i), yIn3)), module, SickoCrosser4L::IN_C_INPUT+i));
			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xs+(x*i), yIn4)), module, SickoCrosser4L::IN_D_INPUT+i));
			
			addParam(createParamCentered<SickoKnob>(mm2px(Vec(xs+(x*i), yKn)), module, SickoCrosser4L::XFD_PARAM+i));
			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xs+(x*i), yMod)), module, SickoCrosser4L::MOD_XFD_INPUT+i));

			addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(xs+(x*i), yOut)), module, SickoCrosser4L::OUT_OUTPUT+i));
		
			/*if (i != 3)
				addParam(createLightParamCentered<VCVLightLatch<MediumSimpleLight<YellowLight>>>(mm2px(Vec(xLink+(x*i), yLink)), module, SickoCrosser4L::LINK_PARAM+i, SickoCrosser4L::LINK_LIGHT+i));
			*/
		}
	}
};

Model* modelSickoCrosser4L = createModel<SickoCrosser4L, SickoCrosser4LWidget>("SickoCrosser4L");