								...
								if (trigValue[track] >= 1.f && prevTrigValue[track] < 1.f) {
									...
									if (probSetup[track] >= probValue) {
										//for (int c = 0; c < chan; c++) {
											...
											out = ( inputs[IN_INPUT+track].getVoltage() * params[SCALE_PARAM+track].getValue() ) + params[OFFSET_PARAM+track].getValue();
											...
											outputs[OUT_OUTPUT+track].setVoltage(out);
										//}
									}					
								}
...
struct Holder8LDebugDisplay : TransparentWidget {
	Holder8L *module;
	int frame = 0;
	Holder8LDebugDisplay() {
	}
...
struct Holder8LWidget : ModuleWidget {
	Holder8LWidget(Holder8L* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/Holder8L.svg")));

		/*
		{
			Holder8LDebugDisplay *display = new Holder8LDebugDisplay();
		...
		*/
		...
		for (int track = 0; track < MAX_TRACKS; track++) {
			...
			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xTrg, yStart+(track*yDelta))), module, Holder8L::TRIG_INPUT+track));

			addParam(createLightParamCentered<VCVLightLatch<MediumSimpleLight<BlueLight>>>(mm2px(Vec(xMode, yStart+(track*yDelta))), module, Holder8L::MODE_SWITCH+track, Holder8L::MODE_LIGHT+track));

			addInput(createInputCentered<SickoInPort>(mm2px(Vec(xIn, yStart+(track*yDelta))), module, Holder8L::IN_INPUT+track));

			addParam(createParamCentered<SickoTrimpot>(mm2px(Vec(xProb, yStart+(track*yDelta))), module, Holder8L::PROB_PARAM+track));

			addParam(createParamCentered<SickoSmallKnob>(mm2px(Vec(xScale, yStart+(track*yDelta))), module, Holder8L::SCALE_PARAM+track));

			addParam(createParamCentered<SickoTrimpot>(mm2px(Vec(xOffset, yStart+(track*yDelta))), module, Holder8L::OFFSET_PARAM+track));

			addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(xOut, yStart+(track*yDelta))), module, Holder8L::OUT_OUTPUT+track));
		}
	}

	void appendContextMenu(Menu* menu) override {
		Holder8L* module = dynamic_cast<Holder8L*>(this->module);
...
Model* modelHolder8L = createModel<Holder8L, Holder8LWidget>("Holder8L");