							...
							if (trigValue >= 1.f && prevTrigValue < 1.f) {
								...
								if (probSetup >= probValue) {
									//for (int c = 0; c < chan; c++) {
										...
										out = ( inputs[IN_INPUT].getVoltage() * params[SCALE_PARAM].getValue() ) + params[OFFSET_PARAM].getValue();
...
struct HolderCompactLDebugDisplay : TransparentWidget {
	HolderCompactL *module;
	int frame = 0;
	HolderCompactLDebugDisplay() {
	}
...
struct HolderCompactLWidget : ModuleWidget {
	HolderCompactLWidget(HolderCompactL* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/HolderCompactL.svg")));

		/*
		{
			HolderCompactLDebugDisplay *display = new HolderCompactLDebugDisplay();
		...
		*/
		...
		addParam(createLightParamCentered<VCVLightLatch<MediumSimpleLight<BlueLight>>>(mm2px(Vec(xCenter, 16.4)), module, HolderCompactL::MODE_SWITCH, HolderCompactL::MODE_LIGHT));

		addInput(createInputCentered<SickoInPort>(mm2px(Vec(xCenter, 28.8)), module, HolderCompactL::TRIG_INPUT));

		addInput(createInputCentered<SickoInPort>(mm2px(Vec(xCenter, 45.5)), module, HolderCompactL::IN_INPUT));

		addParam(createParamCentered<SickoTrimpot>(mm2px(Vec(xCenter, 61)), module, HolderCompactL::PROB_PARAM));

		addParam(createParamCentered<SickoKnob>(mm2px(Vec(xCenter, 74.5)), module, HolderCompactL::SCALE_PARAM));

		addParam(createParamCentered<SickoSmallKnob>(mm2px(Vec(xCenter, 88.7)), module, HolderCompactL::OFFSET_PARAM));

		addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(xCenter, 104.1)), module, HolderCompactL::OUT_OUTPUT));
		addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(xCenter, 117.5)), module, HolderCompactL::TRIG_OUTPUT));

	}

	void appendContextMenu(Menu* menu) override {
		HolderCompactL* module = dynamic_cast<HolderCompactL*>(this->module);
...
Model* modelHolderCompactL = createModel<HolderCompactL, HolderCompactLWidget>("HolderCompactL");