		...
		//if (chanVca == 1) {
			...
			//for (int c = 0; c < chan; c++) {
				...
				//if (polyOuts) {
					...
					out[LEFT] = inputs[IN_INPUT+LEFT].getVoltage() * vcaValue;
					out[RIGHT] = inputs[IN_INPUT+RIGHT].getVoltage() * vcaValue;
					...
					outputs[OUT_OUTPUT+LEFT].setVoltage(out[LEFT]);
					outputs[OUT_OUTPUT+RIGHT].setVoltage(out[RIGHT]);
...
struct SickoAmpLWidget : ModuleWidget {
	SickoAmpLWidget(SickoAmpL* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/SickoAmpL.svg")));
		...
		addInput(createInputCentered<SickoInPort>(mm2px(Vec(x, yIn1)), module, SickoAmpL::IN_INPUT+LEFT));
		addInput(createInputCentered<SickoInPort>(mm2px(Vec(x, yIn2)), module, SickoAmpL::IN_INPUT+RIGHT));

		addParam(createParamCentered<SickoTrimpot>(mm2px(Vec(x, yVca)), module, SickoAmpL::VCA_PARAM));
		addParam(createParamCentered<SickoSmallKnob>(mm2px(Vec(x, yVcaAtnv)), module, SickoAmpL::CV_ATNV_PARAM));
		addInput(createInputCentered<SickoInPort>(mm2px(Vec(x, yVcaIn)), module, SickoAmpL::CV_INPUT));
	
		addParam(createLightParamCentered<VCVLightLatch<MediumSimpleLight<RedLight>>>(mm2px(Vec(x, yLimSw)), module, SickoAmpL::LIMIT_SWITCH, SickoAmpL::LIMIT_LIGHT));
		addParam(createParamCentered<SickoSmallKnob>(mm2px(Vec(x, yLimKnob)), module, SickoAmpL::LIMIT_PARAM));

		addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(x, yOut1)), module, SickoAmpL::OUT_OUTPUT+LEFT));
		addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(x, yOut2)), module, SickoAmpL::OUT_OUTPUT+RIGHT));

	}

	/*
	void appendContextMenu(Menu *menu) override {
	   	SickoAmpL *module = dynamic_cast<SickoAmpL*>(this->module);
			assert(module);
...
Model* modelSickoAmpL = createModel<SickoAmpL, SickoAmpLWidget>("SickoAmpL");