...
struct MultiRouterML : Module {
	...
	MultiRouterML() {
		...
		configInput(IN_LEFT_INPUT, "In");
		//configInput(IN_RIGHT_INPUT, "Right");

		configOutput(OUT_LEFT_OUTPUT, "Out1");
		//configOutput(OUT_RIGHT_OUTPUT, "Out1 R");
		configOutput(OUT_LEFT_OUTPUT+1, "Out2");
		//configOutput(OUT_RIGHT_OUTPUT+1, "Out2 R");
		configOutput(OUT_LEFT_OUTPUT+2, "Out3");
		//configOutput(OUT_RIGHT_OUTPUT+2, "Out3 R");
		configOutput(OUT_LEFT_OUTPUT+3, "Out4");
		//configOutput(OUT_RIGHT_OUTPUT+3, "Out4 R");
		configOutput(OUT_LEFT_OUTPUT+4, "Out5");
		//configOutput(OUT_RIGHT_OUTPUT+4, "Out5 R");
		configOutput(OUT_LEFT_OUTPUT+5, "Out6");
		//configOutput(OUT_RIGHT_OUTPUT+5, "Out6 R");
		configOutput(OUT_LEFT_OUTPUT+6, "Out7");
		//configOutput(OUT_RIGHT_OUTPUT+6, "Out7 R");
		configOutput(OUT_LEFT_OUTPUT+7, "Out8");
	...
	void process(const ProcessArgs& args) override {
		...
		for (int out = 0; out < 8; out++) {
			...
			if (outputs[OUT_LEFT_OUTPUT+out].isConnected()) {
...
struct MultiRouterMLWidget : ModuleWidget {
	MultiRouterMLWidget(MultiRouterML* module) {
		setModule(module);
		setPanel(createPanel(asset::plugin(pluginInstance, "res/MultiRouterML.svg")));
		...
		*/
		...
		const float xOuts = 19.1;
		//const float yIns = 15.6;
		const float yOuts = 37.1;
		...
		constexpr float ys = 48;
		constexpr float yShift = 10;

		addParam(createParamCentered<CKSS>(mm2px(Vec(xLeft, yCvSw)), module, MultiRouterML::MODE_SWITCH));
		addInput(createInputCentered<SickoInPort>(mm2px(Vec(xLeft, yCvSwIn)), module, MultiRouterML::TRG_CV_INPUT));
		addParam(createParamCentered<CKSSThree>(mm2px(Vec(xDir, yDir)), module, MultiRouterML::DIR_SWITCH));

		addParam(createParamCentered<SickoTrimpot>(mm2px(Vec(xLeft, yXfd)), module, MultiRouterML::XFD_PARAM));

		addParam(createParamCentered<SickoTrimpot>(mm2px(Vec(xLeft, yRstKn)), module, MultiRouterML::RST_PARAM));
		addInput(createInputCentered<SickoInPort>(mm2px(Vec(xLeft, yRstIn)), module, MultiRouterML::RST_INPUT));

		addInput(createInputCentered<SickoInPort>(mm2px(Vec(xInL, yIn)), module, MultiRouterML::IN_LEFT_INPUT));
		//addInput(createInputCentered<SickoInPort>(mm2px(Vec(xInR, yIn)), module, MultiRouterML::IN_RIGHT_INPUT));

		addParam(createParamCentered<SickoSmallKnob>(mm2px(Vec(xOuts, yOuts)), module, MultiRouterML::OUTS_PARAM));

		for (int i = 0; i < 8; i++) {
			addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(xInL, ys+(i*yShift))), module, MultiRouterML::OUT_LEFT_OUTPUT+i));
			//addOutput(createOutputCentered<SickoOutPort>(mm2px(Vec(xInR, ys+(i*yShift))), module, MultiRouterML::OUT_RIGHT_OUTPUT+i));
			addChild(createLightCentered<SmallLight<GreenLight>>(mm2px(Vec(xLight, ys+(i*yShift)-4)), module, MultiRouterML::OUT_LIGHT+i));
		}
	...
	void appendContextMenu(Menu* menu) override {
		MultiRouterML* module = dynamic_cast<MultiRouterML*>(this->module);
...
Model* modelMultiRouterML = createModel<MultiRouterML, MultiRouterMLWidget>("MultiRouterML");